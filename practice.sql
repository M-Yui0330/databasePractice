...
SELECT LEFT(code, 1) AS first_letter, name FROM countries;
...
SELECT region, AVG(life_expectancy) AS avg_life, AVG(population) AS avg_pop FROM countries GROUP BY region;

-- 問22
-- 全ての地方の最長寿命、最大人口を表示してください。(NULLも表示)
SELECT region, MAX(life_expectancy) AS max_life, MAX(population) AS max_pop FROM countries GROUP BY region;
...
SELECT c.name AS country_name, cl.language, ci.name AS city_name FROM countries c
JOIN countrylanguages cl ON c.code = cl.country_code
JOIN cities ci ON c.code = ci.country_code;

-- 問27
-- 全ての有名人を出力してください。左側外部結合を使用して国名なし（country_codeがNULL）も表示してください。
SELECT ce.name, co.name AS country_name FROM celebrities ce LEFT JOIN countries co ON ce.country_code = co.code;

-- 問28
-- 全ての有名人の名前,国名、第一言語を出力してください。
SELECT ce.name, co.name AS country_name, cl.language
FROM celebrities ce
JOIN countries co ON ce.country_code = co.code
JOIN countrylanguages cl ON co.code = cl.country_code
WHERE cl.is_official = 'T';

-- 問29
-- 全ての有名人の名前と国名をに出力してください。 ただしテーブル結合せずサブクエリを使用してください。
SELECT name, (SELECT name FROM countries WHERE code = ce.country_code) AS country_name FROM celebrities ce;
...
SELECT COUNT(*) FROM celebrities WHERE birth BETWEEN '1991-01-01' AND '1991-12-31'
UNION
SELECT COUNT(*) FROM celebrities WHERE birth BETWEEN '1981-01-01' AND '1981-12-31';

-- 問32
-- 有名人の出身国の平均年齢を高い方から順に表示してください。ただし、FROM句はcountriesテーブルとしてください。
SELECT c.name AS country_name, AVG(ce.age) AS avg_age
FROM countries c
